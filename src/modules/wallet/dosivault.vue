...
onMounted(() => {
    ...
    initParamsForDosiVault()
})
async function initParamsForDosiVault() {
...
function suggest() {
    ...
    if (window.dosiVault) {
        // @ts-ignore
        window.dosiVault.experimentalSuggestChain(JSON.parse(conf.value)).catch(e => {
            error.value = e
        })
    }
}
...
<template>
    <div class="bg-base-100 p-4 rounded text-center">
        <div class="flex">
            <select v-model="selected" class="select select-bordered mx-5" @change="initParamsForDosiVault">
                <option v-for="c in dashboard.chains" :value="c">
                    {{ c.chainName }}
                </option>
            </select>
            <button class="btn !bg-yes !border-yes text-white px-10" @click="suggest">Add {{ selected.chainName }} TO DosiVault Wallet</button>
        </div>
        <div class="text-main mt-5">
            <textarea v-model="conf" class="textarea textarea-bordered w-full" rows="15"></textarea>
        </div>
        <div class="mt-4 mb-4">
            If the chain is not officially support on DosiVault, you can submit these parameters to enable DosiVault.
        </div>
    </div>
</template>
